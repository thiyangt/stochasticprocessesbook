...
Let's assume that the soil moisture condition on a given day depends only on the moisture condition of the previous day. In this case conditional probability $P(X_{n+1}=j|X_n=i)$ actually depend on $n$. The probability of moving wet to wet ($P(X_{n+1}=2|X_n=2)$) is not same for the whole year. This probability is small during the dry season and very high during the rainy season. If you are living in a country with four seasons then this probability vary according to the seasons winter, summer, spring and autumn. Hence, this is a non-stationary discrete-parameter discrete-state space Markov chain process. We can use stationary Markov chain only for a short period of time. 
